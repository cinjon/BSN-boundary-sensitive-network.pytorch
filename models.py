...
def _get_module(key):
    return {
        # 'timecycle': CycleTime,
        'corrflow': (CorrFlowModel, CorrFlowRepresentation,
                     corrflow_img_loading_func)
    }.get(key)
...
def partial_load(checkpoint_path, model):
    ...
    print(len(pretrained_dict), len(model_dict), len(set(pretrained_dict.keys()).intersection(set(model_dict.keys()))))
    print('***')
...
class TEM(torch.nn.Module):
    def __init__(self, opt):
        ...
        if self.do_representation:
            model, representation, _ = _get_module(
                opt['representation_module'])
